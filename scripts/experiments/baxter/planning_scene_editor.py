    ...
    def permute_obstacles(self, pose_dict):
        for name in pose_dict.keys():
            ...
            if self._obstacles[name]['is_mesh']:
                ...
                # try loading mesh, if not successful, then just add box
                # there is some trouble when loading the mesh of Pepsi file, not sure what the problem is
                # because it worked fine on other systems. Probably due to version
                try:
